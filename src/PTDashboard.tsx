...
interface WorkoutsPageProps {
  workouts: Workout[];
  onSave: (editingId: string | null, form: { name: string; exercises: Exercise[] }) => void;
...
function WorkoutsPage({ workouts, onSave, onDelete }: WorkoutsPageProps) {
  ...
  const [form, setForm] = useState<{ name: string; exercises: Exercise[] }>({ name: "", exercises: [] });
...
export default function PTDashboard() {
  ...
  const saveWorkout = (editingId: string | null, form: { name: string; exercises: Exercise[] }) => {
    if (editingId) setWorkouts(prev => prev.map(w => w.id === editingId ? { ...w, ...form } : w));
    else setWorkouts(prev => [...prev, { id: "w" + Date.now(), ...form }]);
  };
